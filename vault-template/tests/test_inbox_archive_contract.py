...
def test_seed_copies_non_inbox_file(vault: Path) -> None:
    source = vault / "Root Source.md"
    source.write_text("# Root\n", encoding="utf-8")

    result = run_json(["seed", "--source", "Root Source.md", "--mode", "structure"])

    assert "commit_paths" not in result
    assert (vault / result["source"]).read_text(encoding="utf-8") == "# Root\n"
...
def test_seed_reruns_after_inbox_file_was_moved(vault: Path) -> None:
    source = vault / "inbox" / "Source File.md"
    source.write_text("# Source\n", encoding="utf-8")
    first = run_json(["seed", "--source", "inbox/Source File.md", "--mode", "structure"])

    second = run_json(["seed", "--source", "inbox/Source File.md", "--mode", "structure"])
...
def test_seed_rejects_existing_archive_when_inbox_file_exists(vault: Path) -> None:
    source = vault / "inbox" / "Source File.md"
    source.write_text("# Source\n", encoding="utf-8")
    run_json(["seed", "--source", "inbox/Source File.md", "--mode", "structure"])
    source.write_text("# Different\n", encoding="utf-8")

    exit_code, failure = invoke_json(
        ["seed", "--source", "inbox/Source File.md", "--mode", "structure"]
    )
