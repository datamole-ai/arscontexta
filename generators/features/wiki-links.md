...
`Source:` footer links are stricter than ordinary knowledge links. Prefer the concrete archived source path from lean pipeline state, for example `archive/<date>-<batch>.md`. `/verify` uses Obsidian CLI for unresolved ordinary links and runtime validation for deterministic artifact checks.
