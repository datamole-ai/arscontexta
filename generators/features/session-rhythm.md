...
2. **Commit changes** — Every change must be committed. Nothing persists without this. `/process` produces a single `pipeline: {batch_id}` commit at the end of the run; manual work and ad-hoc edits require an explicit `git commit` with a clear message describing what changed and why.
