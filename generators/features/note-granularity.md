...
**The downstream chain is shared.** Both pipelines feed into the same connection-finding plus backward-reconsideration (/connect) and verification (/{DOMAIN:verify}) phases. These phases select behavior based on the `granularity` field.
```
