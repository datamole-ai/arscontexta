...
ERROR: No queue file found. Checked:
  - {config.ops_dir}/queue.yaml
  - {config.ops_dir}/queue/queue.yaml
  - {config.ops_dir}/queue/queue.json
```
Stop.
...
```bash
ls -d {config.ops_dir}/queue/archive/*-{batch_id}/ 2>/dev/null
```

The pattern is `{config.ops_dir}/queue/archive/{date}-{batch_id}/`.

- If found: use the existing folder
- If not found: create `{config.ops_dir}/queue/archive/$(date -u +"%Y-%m-%d")-{batch_id}/`
...
```bash
mv {config.ops_dir}/queue/{batch_id}.md "$ARCHIVE_DIR/"
mv {config.ops_dir}/queue/{batch_id}-*.md "$ARCHIVE_DIR/" 2>/dev/null
```
...
**No {config.ops_dir}/derivation-manifest.md:** Use universal vocabulary for all output (same fallback as all pipeline skills).
