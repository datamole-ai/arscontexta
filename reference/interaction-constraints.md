...
6. `semantic_search == false` (Primitive 10)
   → BLOCK: "Semantic search is INVARIANT. Processing skills (/connect, /verify, /structure, /seed) call `qmd query` via Bash directly. It cannot be disabled at the config level; if qmd is not installed, the wiring is still generated and the install is surfaced as a required next step."
