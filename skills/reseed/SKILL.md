...
context: fork
allowed-tools: Read, Write, Edit, Grep, Glob, Bash, mcp__qmd__query, mcp__qmd__get, mcp__qmd__multi_get, AskUserQuestion
...
3. **Query the research graph** -- use `mcp__qmd__query` to search for claims relevant to the friction. Fall back to `mcp__qmd__query`. If MCP is unavailable, use qmd CLI (`qmd query`, then `qmd vsearch`). Fall back to reading bundled reference files directly only if both MCP and qmd CLI are unavailable.
