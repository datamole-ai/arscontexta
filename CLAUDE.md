...
- `.claude-plugin/plugin.json` owns the generator version, `vault-tooling/pyproject.toml` owns the runtime version, and `template/.second-brain` owns the template version while mirroring the other two. Keep them aligned with `tests/test_version_manifest.py`.
